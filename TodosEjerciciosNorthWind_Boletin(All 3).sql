...
*/
SELECT a.CustomerID, a.ContactName, SUM(c.Quantity) "Produtos comprados"
FROM customers a JOIN orders b ON a.CustomerID = b.CustomerID JOIN orderdetails c ON b.OrderID = c.OrderID
GROUP BY a.CustomerID;
...
/*
Ejercicio 319
Muestra un listado con las ventas totales de cada vendedor por año.
*/
SELECT a.EmployeeID, a.FirstName, sum(c.Quantity) "Productos vendidos"
FROM employees a JOIN orders b ON a.EmployeeID = b.EmployeeID JOIN orderdetails c ON b.OrderID = c.OrderID
GROUP BY a.EmployeeID;

/*
Ejercicio 320
Muestra un listado con las ventas totales por vendedor y cliente en un año determinado.
*/
SELECT a.EmployeeID, a.FirstName, sum(c.Quantity) "Productos vendidos", d.ContactName "Nombre cliente a quien vendio", YEAR(b.orderDate) "Año"
FROM employees a JOIN orders b ON a.EmployeeID = b.EmployeeID JOIN orderdetails c ON b.OrderID = c.OrderID JOIN customers d ON b.CustomerID = d.CustomerID
GROUP BY a.EmployeeID, d.CustomerID, YEAR(b.orderDate)
ORDER BY 1,4;

/*
Ejercicio 321
Muestra un resumen de las ventas totales por cliente y categoría de productos en un año
determinado.
*/
SELECT a.CustomerID, a.ContactName, SUM(c.Quantity) "Produtos comprados", e.CategoryName "Categoría", Year(b.orderDate) "Año"
FROM customers a JOIN orders b ON a.CustomerID = b.CustomerID JOIN orderdetails c ON b.OrderID = c.OrderID JOIN products d ON c.ProductID = d.ProductID JOIN categories e ON d.CategoryID = e.CategoryID
GROUP BY 1,4,5
ORDER BY 1,4,5;
